...
public class FoamsnareEffect implements AlchemicalEffect {
    ...
    public void detonate(UseOnContext context) {
        ...
        PlacementPattern pattern = new SpreadPattern(level, pos, 80, PlacementSet.BLOCK_REPLACEABLE);
        new PlacementSet(level).addPattern(pattern).cull(PlacementSet.BLOCK_REPLACEABLE).placeImmediate(AlchemineBlocks.FOAM_SNARE, true);
