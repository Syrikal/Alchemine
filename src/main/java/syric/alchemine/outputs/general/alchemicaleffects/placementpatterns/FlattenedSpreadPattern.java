...
public class FlattenedSpreadPattern implements PlacementPattern {
    ...
    public FlattenedSpreadPattern(Level level, BlockPos origin, int numberOfBlocks, PlacementFilter filter, double flatFactor) {
        this.level = level;
        this.origin = origin;
            ...
            for (Map.Entry<BlockPos, Integer> entry : blockFillingMap.entrySet()) {
                ...
                if (entry.getValue() > 1) {
                    ...
                    for (Direction direction : Direction.allShuffled(random)) {
                        if (direction.getAxis() == Direction.Axis.Y && random.nextDouble() > flatFactor) {
                            continue;
                        }
